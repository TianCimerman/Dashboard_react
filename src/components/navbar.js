...
const handleIframeToggle2 = async () => {
  ...
  return (
    <>
      <nav className="navbar absolute top-0 right-0 w-[6.5rem] h-full flex justify-between items-center p-4 bg-gray-800 text-white" style={{ backgroundColor: 'hsl(218, 46%, 8%)' }}>
        <ul className="navbar-links flex justify-between items-center h-full flex-col pt-[2.8rem] pb-[2.8rem]">
          ...
          <li>
            ...
            <button onClick={handleIframeToggle} className="hover:text-gray-500 transition duration-300 ease-in-out">
              <FontAwesomeIcon icon={faChartLine} className="text-[3.5rem]" />
            </button>
          </li>
          <li>
            <Link href="/feder">
              <FontAwesomeIcon icon={faPaw} className="text-[3.5rem]" />
            </Link>
          </li>
        </ul>
      </nav>
...
{isIframeVisible && (
  <div className='absolute top-0 left-0 w-[94.1%] h-full z-50'>
    <iframe
...
{loadingIframe && (
  <div className="inset-0 z-[999] flex items-center justify-center  bg-opacity-70 absolute top-0 left-0 w-[94.1%] h-full z-50" style={{ backgroundColor: 'rgb(48, 57, 70)' }}>
